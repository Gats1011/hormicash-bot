  ...
  <div style="max-width:560px;margin:0 auto;padding:32px 16px">
    ...
    <div style="background:#ffffff;border-radius:16px;padding:28px 24px;box-shadow:0 2px 12px rgba(0,0,0,0.06);margin-bottom:16px">
      <div style="font-size:22px;font-weight:800;color:#1a1f36;margin-bottom:8px">
        ${diasSinRegistrar === 0 ? `¡Hola ${nombre}! 👋` : diasSinRegistrar <= 3 ? `${nombre}, llevas ${diasSinRegistrar + 1} días sin registrar 👀` : `${nombre}, más de una semana sin registrar 🔥`}
      </div>
      <div style="font-size:15px;color:#4a5568;line-height:1.6;margin-bottom:20px">
        ${diasSinRegistrar === 0 ? `Hoy no has registrado ningún gasto todavía.` : diasSinRegistrar <= 3 ? `Llevas ${diasSinRegistrar + 1} días sin registrar tus gastos. Este mes ya llevas <strong>${cur} ${totalMes.toFixed(0)}</strong> registrado.` : `Han pasado más de 7 días sin que registres ningún gasto.`}
      </div>
      ${totalMes > 0 ? `<div style="background:#f4f6fb;border-radius:10px;padding:14px 16px;margin-bottom:20px;display:flex;justify-content:space-between;align-items:center"><div style="font-size:13px;color:#8492a6">Registrado este mes</div><div style="font-size:20px;font-weight:800;color:#ef4444">${cur} ${totalMes.toFixed(0)}</div></div>` : ''}
      <a href="https://hormicash.web.app" style="display:block;text-align:center;background:linear-gradient(135deg,#f59e0b,#22c55e);color:white;text-decoration:none;padding:14px 24px;border-radius:12px;font-size:15px;font-weight:700">Ver mi dashboard →</a>
    </div>
...
async function verificarLimite(telefono, categoria, montoNuevo) {
  try {
    ...
    const limite = (userData.limites || {})[categoria];
...
async function verificarAlertaContextual(telefono, categoria, montoNuevo) {
  try {
    ...
    snapshot.forEach(doc => {
      const d = doc.data(); totalMes += d.monto;
    ...
    if (promedioDiario > 0 && totalHoy > promedioDiario * 1.5) {
      return { catDisplay: CATEGORIAS_DISPLAY[categoria]||categoria, totalHoy: totalHoy.toFixed(2), promedio: promedioDiario.toFixed(2) };
    }
    return null;
  } catch(e) { return null; }
}
...
async function actualizarGastoTarjeta(telefono, monto) {
  try {
    ...
    const snap = await userRef.get(); if (!snap.exists) return;
    const gastoActual = (snap.data().tarjeta?.gasto_actual || 0) + monto;
...
function parsearMultiplesMovimientos(texto) {
  const partes = texto.split(/\s+(?:y|e|,)\s+/gi);
...
function parsearMovimiento(texto) {
  ...
  if (ingresosKeywords.some(k => lower.includes(k))) return { monto, tipo:'ingreso', categoria:'ingreso', label: limpiarLabel(texto) };
  const cats = {
...
setInterval(() => {
  const ahora = new Date();
  const horaUTC = ahora.getUTCHours(), minUTC = ahora.getUTCMinutes();
...
app.post('/api/analisis-mes', async (req, res) => {
  try {
    ...
    console.error('Error análisis mes:', e);
    res.status(500).json({ error: e.message });
  }
});

// ── ENDPOINT RECATEGORIZACIÓN ─────────────────────────────────────
...
app.get('/test-email', async (req,res) => {
  const email=req.query.email; if(!email) return res.send('Falta ?email=tucorreo@gmail.com');
  try { await enviarEmailRecordatorio(email,'Stefano',2,1165); res.send(`✅ Email enviado a ${email}`); } catch(e) { res.send(`❌ Error: ${e.message}`); }
});

app.get('/usuarios-emails', async (req,res) => {
  try { const snap=await db.collection('usuarios').get(); const usuarios=[]; snap.forEach(d=>{const data=d.data();if(data.email)usuarios.push({nombre:data.nombre?.split(' ')[0]||'Usuario',email:data.email,plan:data.plan||'free',telefono:data.telefono||''});}); res.json({total:usuarios.length,usuarios}); } catch(e) { res.json({error:e.message}); }
});

app.get('/enviar-recordatorio-todos', async (req,res) => {
  if(req.query.clave!==process.env.ADMIN_SECRET) return res.send('❌ No autorizado');
  try { const snap=await db.collection('usuarios').get(); let enviados=0,errores=0,resultados=[]; for(const d of snap.docs){const data=d.data();if(!data.email)continue;const nombre=data.nombre?.split(' ')[0]||'Usuario';try{await enviarEmailRecordatorio(data.email,nombre,1,0);enviados++;resultados.push({email:data.email,status:'✅'});await new Promise(r=>setTimeout(r,200));}catch(e){errores++;resultados.push({email:data.email,status:`❌ ${e.message}`});}} res.json({enviados,errores,resultados}); } catch(e) { res.json({error:e.message}); }
});

// — PROXY CLAUDE API (se mantiene por si acaso) —
